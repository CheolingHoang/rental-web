...
export default function ProjectDetailsAndFeedbackPage() {
  ...
  // FIX LỖI LINK: Tự động lấy tên miền hiện tại đang chạy (localhost hoặc Vercel)
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      <div className="bg-[#0a0a0a] border border-white/10 rounded-[24px] p-6 md:p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="space-y-6 relative z-10">
          ...
          <div 
            className={`border rounded-xl p-4 flex items-center justify-between cursor-pointer transition-all ${allowDownload ? 'bg-indigo-500/5 border-indigo-500/30' : 'bg-white/5 border-white/10 hover:border-white/20'}`} 
            onClick={handleToggleDownload}
          >
            <div className="flex items-start gap-3">
              <div className={`p-2 rounded-lg ${allowDownload ? 'bg-indigo-500/20 text-indigo-400' : 'bg-white/10 text-zinc-400'}`}>
                {allowDownload ? <DownloadCloud className="w-4 h-4" /> : <Lock className="w-4 h-4" />}
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium text-white">Quyền Tải Ảnh Gốc</p>
                  {isSavingToggle && <Loader2 className="w-3 h-3 text-indigo-400 animate-spin" />}
                </div>
                <p className="text-xs text-zinc-500 mt-0.5">{allowDownload ? 'Khách được phép tải xuống' : 'Đang khóa chức năng tải'}</p>
              </div>
            </div>
            
            <div className={`w-10 h-6 rounded-full flex items-center transition-colors p-1 ${allowDownload ? 'bg-indigo-500' : 'bg-black/50 border border-white/10'}`}>
